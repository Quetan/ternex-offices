...
const Home: FC = () => (
	<Section className='relative mx-0 min-h-dvh w-full max-w-full py-0 xl:py-0'>
		<div
			className='absolute bottom-0 left-0 right-0 top-0 z-0 w-full bg-background bg-cover bg-fixed bg-center bg-no-repeat opacity-50'
			style={{
				backgroundImage: `url(${office})`
			}}></div>

		<div className='relative z-20 flex h-dvh w-full flex-col justify-center'>
			<h1 className='mb-4 max-w-[25ch] text-pretty text-center text-4xl font-black tracking-wide md:text-left xl:text-5xl'>
				Интернет и Сетевые Решения для Вашего Офиса
			</h1>
			<h2 className='mb-6 max-w-[35ch] text-pretty text-center text-2xl font-medium tracking-tight md:text-left xl:text-3xl'>
				Подключите офис к интернету всего от 1100 рублей в месяц!
			</h2>
			<p className='tracking-tigher mb-8 max-w-[50ch] text-pretty text-center text-lg md:text-left xl:text-xl'>
